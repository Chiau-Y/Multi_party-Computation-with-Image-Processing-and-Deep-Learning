...
def search_file(service, download_drive_service_name):
    ...
    results = service.files().list(fields="nextPageToken, files(id, name)", spaces='drive',  # 搜尋你的檔案是否在雲端上，不包含垃圾桶資料
                                   q="name = '" + download_drive_service_name + "' and trashed = false and modifiedTime >'"+time_now_utc+"'",
                                   ).execute()
...
def main(is_download_file_function=False, download_drive_service_name=None, download_file_path=None,
         drive_service_folder_name=None,cred_download_file=None,token_download_file=None):
    """
    ...
    # 認證用
    store = file.Storage(token_download_file)
    creds = store.get()
    if not creds or creds.invalid:
        flow = client.flow_from_clientsecrets(cred_download_file, SCOPES)
    ...
    if is_download_file_function is True:
        print("=====執行下載檔案=====")
        get_folder_id = search_folder(service = service, drive_service_folder_name = drive_service_folder_name)  # 取得資料夾的id，如果沒有會回傳None

        if get_folder_id is None:  # 判斷說 如果沒有資料夾時，會跑這塊搜尋找個雲端上的檔案 PS:這塊部分我沒有針對Google Drive上所建立的檔案去做下載動作，如果指定Google Drive的檔案會有問題
            drive_service_file_id = search_file(service=service, download_drive_service_name=download_drive_service_name)  # 取得要下載
            download_file(service=service, file_id=drive_service_file_id, download_file_path=download_file_path,
                          download_drive_service_name=download_drive_service_name)  # 下載你指定的檔案名稱到本地端
        else:  # 如果有指定資料夾會跑這塊，進行搜尋指定資料夾底下的檔案
            search_folder_files(service=service, download_drive_service_name=download_drive_service_name, folder_id = get_folder_id)  # 透過取得的資料夾id 找尋裡面的所有檔案id
            download_folder_files(service=service, file_id_list=getDownloadFileIdList, download_file_path=download_file_path,  # 執行下載的動作
...
if __name__ == '__main__':
    ...
    for num in range (1):
        main(is_download_file_function=bool(True), drive_service_folder_name=None, 
              download_drive_service_name='Party'+str(num+1)+'.npy', 
              download_file_path=os.getcwd() + '/download/',
              cred_download_file='credentials_'+cred_token_party[num]+'.json',
              token_download_file='token_download_'+cred_token_party[num]+'.pickle') 
