...
def Orinignal_fun(matrix1,matrix4): # matrix_x,fun_e
    ...
    count_zero = 0 # 如果解是0，會造成coefficient長度變短
    for i in range (t+1): #t+1
        unknown = np.poly1d(coefficient)  # 原式
        h_Prime = unknown*fun 
        ans = solve(h_Prime[len(h_Prime)+count_zero-i]-h[len(h)-i],variable[i])      
        try:           
            if not int(ans[0]): # 如果是零
                count_zero += 1
            answer.append(int(ans[0])%prime_num)
...
party2 = np.load('./parties/ImageProcessing/Party2.npy') 
...
party6 = np.load('./parties/ImageProcessing/Party6.npy')  
party7 = np.load('./parties/ImageProcessing/Party7.npy')  
...
party_new = [party1,party2,party3,party4,party5,party6,party7]
...
for i in range (img_size[0]): 
    ...
    for j in range(img_size[1]): 
        ...
        if len(error_party_temp) == 0:            
            ...
            for p in range(7):
                party_new[p][i][j] =  int(round(fun_org(p+1))) % prime_num
...
plt.axis('off') # 不顯示座標軸
np.save('Party2_r_new', party_new[1])
...
plt.axis('off') # 不顯示座標軸        
np.save('Party6_r_new', party_new[5])
