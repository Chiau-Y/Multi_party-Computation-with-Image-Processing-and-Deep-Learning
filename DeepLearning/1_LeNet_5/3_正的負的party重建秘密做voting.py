...
if __name__ == '__main__':
    ...
    for testing_num in range (60000-num,60000):   
        ...
        if (y_train_label[testing_num] == np.argmax(prob_softmax)):
            accuracy+=1
        if (y_train_label[testing_num] == np.argmax(prob_p_softmax)):
            acc_p+=1
        if (y_train_label[testing_num] == np.argmax(prob_n_softmax)):
            acc_n+=1
